import importlib,re,uuid,inspect
...
def is_compatible_type(type: Type) -> bool:
    """Returns `True` if the type is micro_core-compatible."""
    try:
...
def get_input_type(func: Callable) -> Type:
    ...
    if "input" not in type_hints:
        raise ValueError(
            ...
            "For example: `def my_micro_core(input: InputModel) -> OutputModel:`."
        )
...
class MicroCore:
    def __init__(self, func: Union[Callable, str]) -> None:
        ...
        self.uuid = None
        self._name = self.__create_name("MicroCore")
        self._description = ""
        ...
        if inspect.isfunction(self.function):
            ...
            try:
                # Get name
                self._name = self.__create_name(name_to_title(self.function.__name__))
            except Exception:
                pass
            ...
            try:
                # Get name
                self._name = self.__create_name(name_to_title(type(self.function).__name__))
            except Exception:
                pass
...
def get_service_name(micro_core: MicroCore):
    return micro_core.name